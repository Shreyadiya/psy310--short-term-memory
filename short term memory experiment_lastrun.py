...
expName = 'short term memory experiment'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\sspar\\OneDrive\\Desktop\\lab in psychology\\Short term memory task\\short term memory experiment_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('keyinstruction') is None:
        # initialise keyinstruction
        keyinstruction = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='keyinstruction',
        )
    if deviceManager.getDevice('keyy_n_screen') is None:
        # initialise keyy_n_screen
        keyy_n_screen = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='keyy_n_screen',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    textinstruction = visual.TextStim(win=win, name='textinstruction',
        text='Aseries of words will appear, each word lasting for a short viewing time.\nTry to remember each word that is presented to you.\n\nPress space bar to continue.',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    keyinstruction = keyboard.Keyboard(deviceName='keyinstruction')
    ...
    texty_n_screen = visual.TextStim(win=win, name='texty_n_screen',
        text="Following words are testing your recall. Press either 'Y' for yes or 'N' for no.\n\nPress space bar to continue.",
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    keyy_n_screen = keyboard.Keyboard(deviceName='keyy_n_screen')
    ...
    # --- Initialize components for Routine "testrials" ---
    texttestrails = visual.TextStim(win=win, name='texttestrails',
        text='',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
    ...
    thisExp.addData('Welcomeslide.started', globalClock.getTime(format='float'))
    # create starting attributes for keywelcome
    keywelcome.keys = []
    keywelcome.rt = []
    _keywelcome_allKeys = []
    # keep track of which components have finished
    WelcomeslideComponents = [textwelcome, keywelcome]
    for thisComponent in WelcomeslideComponents:
    ...
    thisExp.addData('Welcomeslide.stopped', globalClock.getTime(format='float'))
    # check responses
    ...
    # create starting attributes for keyinstruction
    keyinstruction.keys = []
    keyinstruction.rt = []
    _keyinstruction_allKeys = []
    # keep track of which components have finished
    InstructionsComponents = [textinstruction, keyinstruction]
    ...
    while continueRoutine:
        ...
        if textinstruction.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'textinstruction.started')
            # update status
        ...
        # if keyinstruction is starting this frame...
        if keyinstruction.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'keyinstruction.started')
            # update status
        ...
        if keyinstruction.status == STARTED and not waitOnFlip:
            theseKeys = keyinstruction.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _keyinstruction_allKeys.extend(theseKeys)
            if len(_keyinstruction_allKeys):
                keyinstruction.keys = _keyinstruction_allKeys[-1].name  # just the last key pressed
                keyinstruction.rt = _keyinstruction_allKeys[-1].rt
                keyinstruction.duration = _keyinstruction_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    if keyinstruction.keys != None:  # we had a response
        ...
        thisExp.addData('keyinstruction.duration', keyinstruction.duration)
    thisExp.nextEntry()
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('C:/Users/sspar/OneDrive/word_stimuli.xlsx', selection='0:6'),
    ...
    for thisTrial in trials:
        ...
        textStudyTrial.setColor(WordColour, colorSpace='rgb')
        textStudyTrial.setText(WordItem)
        # keep track of which components have finished
        StudyTrialComponents = [textStudyTrial]
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            if textStudyTrial.status == STARTED:
                ...
                if tThisFlipGlobal > textStudyTrial.tStartRefresh + 2.0-frameTolerance:
                    # keep track of stop time/frame for later
                    textStudyTrial.tStop = t  # not accounting for scr refresh
                    textStudyTrial.tStopRefresh = tThisFlipGlobal  # on global time
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('blankscreen.started', globalClock.getTime(format='float'))
        # keep track of which components have finished
        ...
        for thisComponent in blankscreenComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('blankscreen.stopped', globalClock.getTime(format='float'))
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('y_n_screen.started', globalClock.getTime(format='float'))
    # create starting attributes for keyy_n_screen
    keyy_n_screen.keys = []
    keyy_n_screen.rt = []
    ...
    while continueRoutine:
        ...
        # if keyy_n_screen is starting this frame...
        ...
        if keyy_n_screen.status == STARTED and not waitOnFlip:
            theseKeys = keyy_n_screen.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if len(_keyy_n_screen_allKeys):
                ...
                keyy_n_screen.duration = _keyy_n_screen_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in y_n_screenComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('y_n_screen.stopped', globalClock.getTime(format='float'))
    # check responses
    ...
    if keyy_n_screen.keys != None:  # we had a response
        ...
        thisExp.addData('keyy_n_screen.duration', keyy_n_screen.duration)
    thisExp.nextEntry()
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('C:/Users/sspar/OneDrive/word_stimuli.xlsx'),
    ...
    for thisTrial_2 in trials_2:
        ...
        # --- Prepare to start Routine "testrials" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('testrials.started', globalClock.getTime(format='float'))
        texttestrails.setText(WordItem)
        # create starting attributes for key_resp
        key_resp.keys = []
        key_resp.rt = []
        _key_resp_allKeys = []
        # keep track of which components have finished
        testrialsComponents = [texttestrails, key_resp]
        for thisComponent in testrialsComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        while continueRoutine:
            ...
            # *texttestrails* updates
            
            # if texttestrails is starting this frame...
            if texttestrails.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                texttestrails.frameNStart = frameN  # exact frame index
                texttestrails.tStart = t  # local t and not account for scr refresh
                texttestrails.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(texttestrails, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'texttestrails.started')
                # update status
                texttestrails.status = STARTED
                texttestrails.setAutoDraw(True)
            
            # if texttestrails is active this frame...
            if texttestrails.status == STARTED:
                # update params
                pass
        ...
        # --- Ending Routine "testrials" ---
        for thisComponent in testrialsComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('testrials.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        # the Routine "testrials" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
